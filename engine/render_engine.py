...
- region_engine.py가 생성한 권역 퀵윈 리포트 JSON을 입력으로 받아,
  웹 디자인 스펙(spec/design_html/html/PR2.html, "권역 진단 보고서 팝업")에 맞춘
...
def render_html(rpt):
    ...
    return f"""<!DOCTYPE html><html class="light" lang="ko"><head>
<meta charset="utf-8">
...
tailwind.config = {{ darkMode:"class", theme:{{ extend:{{
  colors:{{ "accent-red":"#E63946","surface-bright":"#fbf9f9","inverse-surface":"#303031",
    "surface-container-lowest":"#ffffff","secondary":"#005db7","on-primary":"#ffffff","error":"#ba1a1a",
    "primary-container":"#003478","on-primary-container":"#7d9fe9","surface-dim":"#dbdad9",
    "surface-container-low":"#f5f3f3","surface-border":"#DCDCDC","surface-container-highest":"#e3e2e2",
    "surface-container":"#efeded","surface-variant":"#e3e2e2","inverse-on-surface":"#f2f0f0",
    "outline":"#747782","surface-light":"#F8F9FA","text-secondary":"#555555","surface-tint":"#395da2",
    "on-surface-variant":"#434751","outline-variant":"#c4c6d2","text-disabled":"#BEBEBE",
    "secondary-container":"#599bfe","on-secondary":"#ffffff","background":"#fbf9f9","surface":"#fbf9f9",
    "on-background":"#1b1c1c","text-primary":"#000000","surface-container-high":"#e9e8e7",
    "primary":"#00204e","on-surface":"#1b1c1c" }},
  borderRadius:{{ "DEFAULT":"0.25rem","lg":"0.5rem","xl":"0.75rem","full":"9999px" }},
  spacing:{{ "sm":"8px","gutter":"24px","base":"4px","xl":"32px","lg":"24px","xs":"4px","md":"16px" }},
  fontFamily:{{ "headline-md":["Hanken Grotesk"],"label-md":["Hanken Grotesk"],"headline-lg":["Hanken Grotesk"],
    "body-sm":["Hanken Grotesk"],"label-sm":["Hanken Grotesk"],"body-lg":["Hanken Grotesk"],
    "body-md":["Hanken Grotesk"] }},
  fontSize:{{ "headline-md":["24px",{{"lineHeight":"32px","fontWeight":"600"}}],
...
<style>
  ...
  @media print {{ .no-print {{ display:none !important; }} .tab-panel {{ display:flex !important; }}
    nav {{ display:none !important; }} body {{ background:#fff; }} }}
</style>
</head>
<body class="bg-background text-on-background font-body-md min-h-screen">
<div class="max-w-6xl mx-auto my-md md:my-lg px-md md:px-0">
<div class="rounded-xl shadow-[0_12px_24px_rgba(0,32,78,0.12)] flex flex-col overflow-hidden bg-surface-container-lowest border border-surface-border">
  <header class="border-b border-surface-border px-lg py-lg flex flex-col md:flex-row md:items-end justify-between gap-md shrink-0">
    <div class="flex items-start gap-md">
      <div class="p-sm bg-primary-container text-on-primary-container rounded-lg flex items-center justify-center shrink-0">
        <span class="material-symbols-outlined text-headline-lg fill">public</span>
      </div>
      <div>
        <div class="flex flex-wrap items-center gap-sm mb-xs">
          <span class="font-label-sm text-label-sm text-text-secondary uppercase tracking-wider">Report ID: {esc(rpt.get("report_id",""))}</span>
          <span class="w-1 h-1 rounded-full bg-surface-border"></span>
          <span class="font-label-sm text-label-sm text-text-secondary">생성: {esc(fmt_dt(rpt.get("created_at","")))}</span>
          <span class="w-1 h-1 rounded-full bg-surface-border"></span>
          <span class="font-label-sm text-label-sm text-text-secondary">베이스라인: {esc(rpt.get("baseline",""))}</span>
        </div>
        <h1 class="font-headline-lg text-headline-lg text-primary m-0">{esc(title)}</h1>
        <p class="font-body-md text-body-md text-on-surface-variant mt-xs mb-0">베이스라인국 대비 권역 내 진출 후보국의 퀵윈(빠른 성과) 진단.</p>
      </div>
    </div>
    <div class="flex items-center gap-sm shrink-0 no-print">
      <button onclick="window.print()" class="flex items-center gap-sm px-md py-sm border border-primary text-primary rounded-lg font-label-md text-label-md hover:bg-surface-light transition-colors">
        <span class="material-symbols-outlined text-[18px]">picture_as_pdf</span> PDF 다운로드
      </button>
    </div>
  </header>
  <nav class="border-b border-surface-border px-lg flex gap-xl shrink-0 overflow-x-auto no-scrollbar no-print">
    {nav}
  </nav>
  <div class="flex-1 overflow-y-auto p-lg bg-surface flex flex-col gap-xl">
    {panels}
  </div>
...
function showTab(key) {{
  document.querySelectorAll('.tab-panel').forEach(p => {{
    ...
    p.classList.toggle('flex', p.dataset.panel === key);
  }});
...
def load_report(region, version=None):
    ...
    if not os.path.exists(path):
        ...
        if not cand:
            raise SystemExit(f"[안내] region '{region}' 리포트 JSON 없음 — 먼저 region_engine.py 실행 필요.")
        path = cand[-1]
